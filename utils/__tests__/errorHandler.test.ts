  ...
  describe("handleAuthError", () => {
    ...
    describe("registration errors", () => {
      it("should return user-friendly message for email-already-in-use", () => {
        const error = new FirebaseError("auth/email-already-in-use", "Email in use");
      ...
      it("should return user-friendly message for weak-password", () => {
        ...
        expect(handleAuthError(error)).toBe("Password must be at least 6 characters");
      });
    ...
    describe("password reset specific errors", () => {
      it("should return user-friendly message for invalid-action-code", () => {
        ...
        expect(handleAuthError(error)).toBe("This password reset link is invalid or has expired");
      });

      it("should return user-friendly message for expired-action-code", () => {
        const error = new FirebaseError("auth/expired-action-code", "Expired action code");
        expect(handleAuthError(error)).toBe(
    ...
    describe("token errors", () => {
      it("should return user-friendly message for invalid-user-token", () => {
        const error = new FirebaseError("auth/invalid-user-token", "Invalid token");
        expect(handleAuthError(error)).toBe("Your session has expired. Please sign in again");
      });

      it("should return user-friendly message for user-token-expired", () => {
        const error = new FirebaseError("auth/user-token-expired", "Token expired");
        expect(handleAuthError(error)).toBe("Your session has expired. Please sign in again");
      });

      it("should return user-friendly message for requires-recent-login", () => {
        const error = new FirebaseError("auth/requires-recent-login", "Recent login required");
        expect(handleAuthError(error)).toBe(
    ...
    describe("unknown error codes", () => {
      ...
      it("should handle completely unrecognized error codes", () => {
        const error = new FirebaseError("auth/some-new-firebase-error", "New error");
        expect(handleAuthError(error)).toBe("An unexpected error occurred. Please try again");
      });
    });
  ...
  describe("handleGenericError", () => {
    ...
    describe("string errors", () => {
      it("should return string error directly", () => {
        expect(handleGenericError("String error message")).toBe("String error message");
      });
    ...
    describe("unknown error types", () => {
      it("should return generic message for null", () => {
        expect(handleGenericError(null)).toBe("An unexpected error occurred. Please try again");
      });
      ...
      it("should return generic message for number", () => {
        expect(handleGenericError(123)).toBe("An unexpected error occurred. Please try again");
      });

      it("should return generic message for boolean", () => {
        expect(handleGenericError(false)).toBe("An unexpected error occurred. Please try again");
      });
