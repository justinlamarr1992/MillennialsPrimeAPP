...
describe("serverAuth", () => {
  ...
  describe("loginToServer", () => {
    ...
    it("throws error on failed login", async () => {
      ...
      await expect(serverAuth.loginToServer("test@example.com", "wrongpassword")).rejects.toThrow(
        "Unauthorized"
      );
    });
  ...
  describe("registerOnServer", () => {
    ...
    it("throws error when registration fails", async () => {
      ...
      await expect(
        serverAuth.registerOnServer({ email: "existing@example.com", password: "password123" })
  ...
  describe("refreshToken", () => {
    ...
    it("throws descriptive error when no refresh token is stored in SecureStore", async () => {
      ...
      await expect(serverAuth.refreshToken()).rejects.toThrow(
